    ...
    useEffect(() => {
        ...
        const draw = (time: number) => {
            ...
            currentUnits.forEach(unit => {
                ...
                // DEBUG: Log only the first unit missing faction (once per render)
                if (!faction && currentUnits.indexOf(unit) === 0) {
                    console.log('[DEBUG] Unit factionId:', unit.factionId, 'Available factions:', currentFactions.map(f => f.id));
                }
